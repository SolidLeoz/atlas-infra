...
: "${EDGE_MQTT_TLS_CA:?missing EDGE_MQTT_TLS_CA}"
: "${EDGE_MQTT_TLS_CERT:?missing EDGE_MQTT_TLS_CERT}"
: "${EDGE_MQTT_TLS_KEY:?missing EDGE_MQTT_TLS_KEY}"
...
# ✅ Hardened: default a false — non saltare la verifica TLS se la variabile non è definita.
# Impostare a true SOLO per self-signed certs in sviluppo.
EDGE_INFLUX_TLS_SKIP_VERIFY="${EDGE_INFLUX_TLS_SKIP_VERIFY:-false}"
